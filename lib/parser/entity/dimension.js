...
class Dimension extends BaseParser{
    ...
    parse(buffer, cursor){
        ...
        if(buffer.hasOwnProperty('dimensionType')){
            ...
            let bits;
            let dimensionType = 1;
            if (buffer['dimensionType']) {
                bits = bitwise.byte.read(buffer['dimensionType']);
                dimensionType = bitwise.byte.write(bitwise.bits.and(bits, [0,0,0,0,1,1,1,1])); //4비트만 연산할수있게 정리    
            }
            /* 
                0 = DimensionLinear 
                1 = DimensionAligned
